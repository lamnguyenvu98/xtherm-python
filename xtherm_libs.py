...
class InfiFrame:
    def __init__(self, height: int = 196, width: int = 256) -> None:
        self.height = height
        self.width = width
        
        self.ZEROC = 273.15
        self.distance_multiplier = 1.0
        self.offset_temp_shutter = 0.0
        self.offset_temp_fpa = 0.0

        self.range = 120
        
        self.cal_00_offset = 390.0
        self.cal_00_fpamul = 7.05
    ...
    ''' Auto-detect thermal camera if it's available '''
    def find_camera(self):
        for i in range(10):
            try:
                ...
                if w == self.width and h == self.height: return cap
            except: pass
        
        raise ValueError("Cannot find camera with given resolution: {} x {}".format(self.width, self.height))
    ...
    def get_temp_table(self, correction, Airtmp, table_offset, distance_adjusted):
        ...
        return ttot + (distance_adjusted * 0.85 - 1.125) * (ttot - Airtmp) / 100.0 + correction
